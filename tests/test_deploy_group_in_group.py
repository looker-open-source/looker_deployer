...
def test_write_groups_in_group_new(mocker):
    ...
    target_sdk.add_group_group.assert_called_once_with(group_id=group_2.id,body=models.GroupIdForGroupInclusion(group_id=1))
...
def test_write_groups_in_group_change(mocker):
    ...
    deploy_group_in_group.write_groups_in_group(source_sdk,target_sdk)
    target_sdk.delete_group_from_group.assert_called_once_with(group_id=group_2.id,deleting_group_id = 1)
    target_sdk.add_group_group.assert_called_once_with(group_id=group_3.id,body=models.GroupIdForGroupInclusion(group_id=1))
